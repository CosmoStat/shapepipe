...
def plot_meanshapes(starcat_path, output_path, nb_pixel, remove_outliers = False,
                    print_fun = lambda x : plot(x)):
    """
    The function that does literally all the plots except from the rho stats ones.
    Meanshapes and histograms mostly.
    """
    # READ FULL STARCAT
    ...
    all_star_shapes = np.array([starcat[2].data['E1_STAR_HSM'],starcat[2].data['E2_STAR_HSM'],
                            2.*starcat[2].data['SIGMA_STAR_HSM']**2])
    all_psf_shapes = np.array([starcat[2].data['E1_PSF_HSM'],starcat[2].data['E2_PSF_HSM'],
    ...
    if remove_outliers == True:
        ...
        print_fun('Nb of outlier stars: %d'%(np.sum(bad_stars)))
        # Remove outlier PSFs
    ...
    print_fun('Bins: e1 residual RMSE: %.6f\n'%(rmse_e1))
    ...
    print_fun('Bins: e2 residual RMSE: %.6f\n'%(rmse_e2))
    ...
    print_fun('Bins: R2 residual RMSE: %.6f\n'%(rmse_r2))
...
def rho_stats(starcat_path,output_path,rho_def='HSC',
    ...
    stilecat = np.rec.fromarrays([w, starcat[hdu_no].data['RA'], starcat[hdu_no].data['DEC'],
    ...
    if rho_def == 'HSC':
        ...
        rho_stats = [Rho1SysTest(), DESRho2SysTest(),
                      DESRho3SysTest(), DESRho4SysTest(), DESRho5SysTest()]
...
@module_runner(input_module=['mccd_merge_starcat_runner'], version='1.0',
               ...
               depends=['numpy', 'mccd_rca','astropy', 'matplotlib', 'stile',
               'treecorr'],
               run_method='serial')
def mccd_plots_runner(input_file_list, run_dirs, file_number_string,
    ...
    plot_rho_stats = config.getboolean('MCCD_MEANSHAPES_PLOTS', 'PLOT_RHO_STATS')
    rho_stat_plot_style = config.get('MCCD_MEANSHAPES_PLOTS', 'RHO_STATS_STYLE')

    nb_pixel = x_nb_bins, y_nb_bins
    starcat_path = input_file_list[0][0]
    output_path = run_dirs['output'] + '/'

    plot_meanshapes(starcat_path, output_path, nb_pixel, remove_outliers,
                    print_fun = lambda x : w_log.info(x))

    if plot_rho_stats is True:
        if has_stile is False or has_treecorr is False:
            ...
            "This was not the case, so the rho stat calculation is aborted."+\
            "For the next time make sure both of the packages are installed."
            warnings.warn(msg)
            w_log.info(msg)
        elif rho_stat_plot_style != 'HSC' and rho_stat_plot_style != 'DEC':
            msg = "The rho stat definition should be HSC or DEC. An unknown "+\
            "definition was used. Rho stat calculation aborted."
            warnings.warn(msg)
            w_log.info(msg)
        else:
            rho_stats(starcat_path, output_path,rho_def=rho_stat_plot_style,
                      print_fun = lambda x : w_log.info(x))
