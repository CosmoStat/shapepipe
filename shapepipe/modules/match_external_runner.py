...
class MatchCat(object):
    ...
    def process(self):
        ...
        external_data, external_ra, external_dec = get_data_ra_dec(
            ...
            self.external_col_match[0], self.external_col_match[1])
        external_coord = SkyCoord(ra=external_ra, dec=external_dec, unit='deg')

        # (loop over inputs)
        for i in range(len(self._input_file_list)):
            data, ra, dec = get_data_ra_dec(self._input_file_list[i], hdu_no,
                                            col_match[0], col_match[1])
            coord = SkyCoord(ra=ra, dec=dec, unit='deg')
            # Todo: cut duplicates

            # Match objects in cat to external cat
            idx, d2d, d3d = match_coordinates_sky(external_coord, coord, nthneighbor=1)
...
def paste_cat_runner(input_file_list, run_dirs, file_number_string,
    ...
    if config.has_option('MATCH_EXTERNAL_RUNNER', 'HDU'):
        hdu_no = config.getlist('MATCH_EXTERNAL_RUNNER', 'HDU')
    else:
        hdu_no = 1
    ...
    if config.has_option('MATCH_EXTERNAL_RUNNER', 'EXTERNAL_HDU'):
        external_hdu_no = config.getlist('MATCH_EXTERNAL_RUNNER', 'EXTERNAL_HDU')
    else:
        external_hdu_no = 1
    ...
    inst = MatchExt(input_file_list, output_path, w_log,
                    tolerance, col_match, hdu_no,
                    external_cat_path, external_col_match,
                    external_col_copy,
