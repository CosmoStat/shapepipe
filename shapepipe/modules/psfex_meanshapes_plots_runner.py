...
@module_runner(input_module=['psfex_merge_starcat_runner'], version='1.0',
               ...
               depends=['numpy','astropy', 'matplotlib'],
               run_method='serial')
def psfex_meanshapes_plots_runner(input_file_list, run_dirs, file_number_string,
    ...
    x_nb_bins = config.getint('PSFEX_MEANSHAPES_PLOTS_RUNNER', 'X_GRID')
    y_nb_bins = config.getint('PSFEX_MEANSHAPES_PLOTS_RUNNER', 'Y_GRID')
    remove_outliers = config.getboolean('PSFEX_MEANSHAPES_PLOTS_RUNNER', 'REMOVE_OUTLIERS')
