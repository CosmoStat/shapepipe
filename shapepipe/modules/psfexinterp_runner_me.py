...
def psfexinterp_runner_me(input_file_list, run_dirs, file_number_string,
    ...
    pos_params = config.getlist('PSFEXINTERP_RUNNER_ME', 'POSITION_PARAMS')
    get_shapes = config.getboolean('PSFEXINTERP_RUNNER_ME', 'GET_SHAPES')
    star_thresh = config.getint('PSFEXINTERP_RUNNER_ME', 'STAR_THRESH')
    chi2_thresh = config.getint('PSFEXINTERP_RUNNER_ME', 'CHI2_THRESH')
