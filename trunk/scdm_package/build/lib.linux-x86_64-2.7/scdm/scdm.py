...
class Param(object):
   ...
   def __init__(self, name, guess_value=0, param_range=[None, None], 
      ...
      self._is_constant = is_constant     # fixed parameter if True
   ...
   def is_constant(self):
      """! 
         Tell whether the parameter value should remain constant during fitting, that is, if the 
...
class Params(object):
   ...
   def __init__(self, param_list=[]):
      ...
      self._free_param_list = [p for p in param_list if not p.is_constant]
   ...
   def free_param_list(self):   
      """!
         Return the list of free parameters objects (non fixed)
         @return the list of free parameters as Param objects
         @see Param
      """
   ...
   @property
   def nb_free_params(self):
      """! 
          Return the number of free (non constant) parameters
          @return the number of non constant parameters
          @see Param    
      """   
      return len(self._free_param_list)
...
class FittingOptions(object):
   ...
   def __init__(self, max_iter=1000, max_fev=1000, max_func_tol=1e-8, all_minima=False,
      ...
      # --- Internal options
      self._dcount_step_inc_threshold = 2    # number of steps in the same direction beyond which the step size should start increasing by a factor <param.step_factor> 
...
class Minimizer(object):
   ...
   def nb_free_params(self):
      ...
      return self.params.nb_free_params
      ...
      try:
         ...
         for iiter in xrange(1, self.fitting_options.max_iter+1):
            ...
            for param in self.params.param_list:
               ...
               # Stop varying if the parameter has been declared as constant (i.e. fixed)
...
#                  #self.logger.log_info('Scaled: %11.6e  Unscaled: %11.6e' %(newScaledParamValue, new_param_value))
               ...
               if np.isnan(delta_func):
                  if param.numerical_error_handler(iiter, param, delta_func, 'delta_func') < 0:
                     ...
                     raise Minimizer.ParamNumericalException(iiter, param, delta_func, 'delta_func', '')
                  else:
                     if __debug__:
                        if self.is_logging_enabled(): 
...
#                        new_param_values[param_index] = new_param_value
               ...
               # The new parameters values become the previous ones...
