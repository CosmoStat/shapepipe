...
def _build_ldac_imhead(img_header):
    ...
    """
    # Build list of 80-char card images (no blank FITS-block padding)
    cards = [str(card).ljust(80)[:80] for card in img_header.cards]
    n_cards = len(cards)
    header_str = "".join(cards)

    # Store as one character string per row; TDIM makes it a 2-D char array
    # so that iterating over data[0][0] yields individual 80-char cards.
...
def _extract_vignets(image_data, x_pos, y_pos, stamp_size):
    ...
    for i, (x, y) in enumerate(zip(x_pos, y_pos)):
        # Convert 1-based SExtractor pixel coords to 0-based numpy indices
        xi = int(round(float(x))) - 1
        yi = int(round(float(y))) - 1

        # Requested slice in image coordinates
        x0, x1 = xi - half, xi + half + 1
        y0, y1 = yi - half, yi + half + 1

        # Clipped slice (stays inside image)
        xc0, xc1 = max(0, x0), min(nx, x1)
        yc0, yc1 = max(0, y0), min(ny, y1)

        # Corresponding destination slice in the vignet array
...
def _build_ldac_objects(cat_data, unique_id, vignets):
    ...
    # Alias columns so downstream configs using SExtractor windowed-position
    # names (XWIN_IMAGE, YWIN_IMAGE, XWIN_WORLD, YWIN_WORLD) work unchanged.
    _aliases = {
        "XWIN_IMAGE": "X_IMAGE",
        "YWIN_IMAGE": "Y_IMAGE",
    ...
    # VIGNET: stored as flattened vector per object; TDIM restores 2-D shape
...
def make_ldac_from_ascii(
    ...
    """
    # Read ASCII catalogue
    cat_data = asc.read(input_cat_path, format="sextractor")
    n_obj = len(cat_data)
    if w_log:
        w_log.info(f"Read {n_obj} objects from {input_cat_path}")

    # Unique per-object ID: tile_id * 1e6 + NUMBER
    # file_number_string like '-301-279'
    parts = file_number_string.lstrip("-").split("-")
    tile_id = int(parts[0]) * 1000 + int(parts[1])  # e.g. 301*1000+279 = 301279
    unique_id = (
        tile_id * 10**6 + np.array(cat_data["NUMBER"], dtype=np.int64)
    )

    # Read tile image: header for exposure history + pixel data for vignets
