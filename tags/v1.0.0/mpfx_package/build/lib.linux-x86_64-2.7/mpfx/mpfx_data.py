...
class MpfxDataset(Dataset):
   ...
   def query(self, master, file_search_pattern, dir_list=[],   
      ...
      file_paths = self._file_matcher.match_files(master)

      #print "*** MPFX query: file_paths: ", file_paths
   ...
   # -----------------------------------------------------------------------------------------------
   def is_fits(self, master, file_path):
      """! 
   ...
   def is_catalog(self, master, file_path):
      ...
      is_catalog = False
      if master.config.has_section("PRIMARY_DATASET.CATALOG_PROPERTIES"):
         if master.config.has_key("FILENAME_PATTERNS",  
                                  "PRIMARY_DATASET.CATALOG_PROPERTIES"):
            filename_patterns = master.config.get_as_list("FILENAME_PATTERNS", 
                                                          "PRIMARY_DATASET.CATALOG_PROPERTIES")
   ...
   def is_image(self, master, file_path):
      ...
      is_image = False
      if master.config.has_section("PRIMARY_DATASET.IMAGE_PROPERTIES"):
         if master.config.has_key("FILENAME_PATTERNS",  
   ...
   def is_star_catalog(self, master, file_path):
      """! Attempt to determine if a file is a star catalog 
           @param master master object instance
           @param file_path absolute file path of a file
           @note by default, it is assumed that the filename contains the keyword "star";
                 to change this behavior, override this method in a sub-class of MPfx
      """
   
      filedir, filename = os.path.split(file_path)

      is_catalog = False
      if master.config.has_section("PRIMARY_DATASET.CATALOG_PROPERTIES.STAR"):
         if master.config.has_key("FILENAME_PATTERNS",  
   ...
   def is_galaxy_catalog(self, master, file_path):
      """! Attempt to determine if a file is a galaxy catalog 
           ...
           @note by default, it is assumed that the filename contains the keyword "gal"; 
                 to change this behavior, override this method in a sub-class of MPfx
      """
      ...
      is_catalog = False
      if master.config.has_section("PRIMARY_DATASET.CATALOG_PROPERTIES.GALAXY"):
         if master.config.has_key("FILENAME_PATTERNS",  
   ...
   def is_star_image(self, master, file_path):
      """! Attempt to determine if a file is a star image 
           ...
           @param file_path absolute file path of a file
           @note by default, it is assumed that the filename contains the keyword "star";
                  the behavior of this method should probably be overriden in a subclass of Mpfx
      ...
      is_image = False
      if master.config.has_section("PRIMARY_DATASET.IMAGE_PROPERTIES.STAR"):
         if master.config.has_key("FILENAME_PATTERNS",  
   ...
   def is_galaxy_image(self, master, file_path):
      ...
      is_image = False
      if master.config.has_section("PRIMARY_DATASET.IMAGE_PROPERTIES.GALAXY"):
         if master.config.has_key("FILENAME_PATTERNS",  
...
class MpfxFileMatcher(object):
   ...
   def match_files(self, master):
      ...
      #print "*** MPFG3: filepaths:", file_paths
   ...
   def match_file(self, master, path, filename, pattern):
      ...
      if matched:
         ...
         if len(self.image_list) >= 0 or self.image_range != [-1, -1]:
            ...
            regex_dash = re.compile("\-(.+)\-(.+)\.")    # xxx-000-00.*  (with epoch index)
            regex_dot = re.compile("\-(.+)\.")           # xxx-000.*     (without epoch index)

            (img_no, epoch) = self._get_img_no_epoch(filename, regex_dash, regex_dot)
            ...
            if img_no is not None:
               ...
               if matched:   
                  ...
                  #print "*** MFG3: calling build_filepath_dico(() with:", self._filepath_dico, path, filename, img_no

                  # --- Populate filepath dictionary per image (with unformatted file paths)
                  self.build_filepath_dico(self._filepath_dico, path, filename, img_no)
               ...
               self.dataset.helper.print_warning(
                     ...
                     "<img_name-img_no-epoch_no.ext> => Ignored".format(filename))

      return matched

     
   # --------------------------------------------------------------------------------------------
   def build_filepath_dico(self, filepath_dico, path, filename, img_no):
      """! 
...
#         tree_list = ["."]
      ...
      #print "*** MPFG3 tree list:", tree_list
      ...
      if '-' in file_prefix:
         ...
         if len(file_components) > 2:
            epoch_index = int(file_components[2])           
         else:
            epoch_index = None   # no epoch index => a catalog
      ...
      #print "*** MPFG3 tree list:", filepath_dico
...
class MpfxMultiEpochFileMatcher(MpfxFileMatcher):
   ...
   """! 
        The reponsibility of this class is to locate all files in the source dataset 
        that match specified criteria (directory list, file pattern, etc.)  
        @note: this should be an inner class but this creates problems when pickling the object
   """
