...
class MpfxJob(Job):      
   ...
   def img_path_dico(self):
      """! @return the file path dictionary with the job. """
   ...
   # -----------------------------------------------------------------------------------------------
   def get_file_paths(self, epoch=None):
      """! 
      ...
      if not path_dico is None:
         if epoch is None:
            return path_dico.values()
         else:
            return [ p for p in path_dico.values()\
   ...
   # -----------------------------------------------------------------------------------------------
   def get_file_path(self, file_type, epoch=None):
      """! 
         Return the path of a given file type. For instance, get_file_path("star_catalog.txt")
         @param file_type type of the file (like "star_catalog.txt")
         @param epoch epoch index or None for any epoch
   ...
   def get_file_components_from_path(self, file_path):
      ...
      filemain, fileext = os.path.splitext(filename)      
