"""! 
   pse_plot.py - Plotting functions.
"""
...
#         plt.close()
   ...
   def make_plots(self, job_result, plot_output_dir, master, plot_prefix=""):
         """! Plots histograms based on generated SExtractor catalog columns """
            ...
            if len(job_result.data_dico) > 0: 
               ...
               for col_name in job_result.data_dico[file_type].keys():
                  ...
                  if plot_prefix != "":
                     ...
                     plot_title_prefix = ""
                  plot_title = "{0}{1} - Image {2:03d}-{3:1d} - /{4} - {5}".format(
   ...
   def make_fragment_plots(self, fragment_data_dico, inner_radius, outer_radius, 
         ...
         plot_title = "Fragment Magnitude - Image {0:03d}-{1:1d} - /{2} - {3}".format( 
                                  job.img_no, job.epoch, branch_tree, file_type)
         ...
         plot_title = "Fragment Distance/centroid - Image {0:03d}-{1:1d} - /{2} - {3}".format( 
                                  job.img_no, job.epoch, branch_tree, file_type)
         ...
         plot_title = "Fragment Counts - Image {0:03d}-{1:1d} - /{2} - {3}".format( 
                                  job.img_no, job.epoch, branch_tree, file_type)
         ...
         plot_title = "Fragment Distance/Magnitude - Image {0:03d}-{1:1d} - /{2} - {3}".format( 
                                  job.img_no, job.epoch, branch_tree, file_type)
