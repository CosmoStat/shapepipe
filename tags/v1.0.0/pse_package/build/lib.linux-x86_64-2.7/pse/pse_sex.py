...
class SExtractorRunner(object):
   ...
   def run_SExtractor(self, file_type, job, worker):
      ...
      try:
         ...
         if se_must_redir_output:
            se_exec_line =\
             "{0} {1} -c {2} -CATALOG_NAME {3} -CHECKIMAGE_NAME {4} -CHECKIMAGE_TYPE {5} -VERBOSE_TYPE {6} > {7}".format(
                 ...
                 se_output_redir_filepath)
         else:
            se_exec_line =\
             "{0} {1} -c {2} -CATALOG_NAME {3} -CHECKIMAGE_NAME {4} -CHECKIMAGE_TYPE {5} -VERBOSE_TYPE {6}".format(
                 se_exec_path, image_filepath, se_config_filepath, se_output_cat_filepath, 
                 se_output_check_filepath, se_check_image_type, se_verbose_type) 
      ...
      if self.helper.file_exists(sex_file_candidate):
         ...
         if self.helper.file_exists(sex_file_candidate):
            ...
            if len(found_files) > 0:
               ...
               sex_filepath = obs_abspath(os.path.join(worker.base_input_dir, "default.sex"))
   ...
   def _get_output_se_catalog_filename(self, image_filepath, job, config):
      ...
      """
      image_path, image_filename = os.path.split(image_filepath)
      se_catalog_prefix, image_filemain_ext = os.path.splitext(image_filename)
   ...
   def _get_output_se_check_filename(self, image_filepath, job, config):
      ...
      """
      image_path, image_filename = os.path.split(image_filepath)
      se_check_prefix, image_filemain_ext = os.path.splitext(image_filename)
   ...
   def _get_output_se_redir_filename(self, image_filepath, job, config):
      ...
      """
      image_path, image_filename = os.path.split(image_filepath)
      se_redir_prefix, image_filemain_ext = os.path.splitext(image_filename)
...
class SExtractorProcessor(object):
   ...
   def process_catalog(self, se_catalog_filepath, se_output_check_filepath, file_type, job, worker):
      ...
      try:
         ...
         #actual_object_count = self.helper._get_nb_objects(image_filepath, stamp_size)
         initial_object_count = self.helper._get_catalog_nb_entries(se_catalog_filepath)
             
         # --- Add extra columns to the catalog
         self._add_extra_colunmns(se_catalog_filepath, file_type,
...
#         deleted_coords.extend(deleted_shifts)
         ...
         final_object_count = self._sort_by_coordinates(se_catalog_filepath, file_type, job, worker)
         ...
         if worker.logging_enabled():
            if se_xform_dico["final_object_count"] > 0:
               worker.logger.log_info_p(
                   "{0} - /{1}/img-{2:03}-{3:1d} - {4} - Catalog {5} processed successfully".format(
                   worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type, 
                   se_catalog_filepath))
            else:
               worker.logger.log_warning_p(
         ...
         if worker.logging_enabled():
            worker.logger.log_error_p(
                "{0} - Some error occurred while processing SExtractor catalog: {1} ({2})".format(
                                                                                worker.name, 
                                                                                job,
                                                                                detail))
            worker.logger.flush()
   ...
   def _add_extra_colunmns(self, se_catalog_filepath, file_type, se_output_check_filepath,  
      ...
      xc_array = catalog.get_col_data(x_image_colname) - 1.0   # SE coordinates start from 1, not 0
      yc_array = catalog.get_col_data(y_image_colname) - 1.0   # SE coordinates start from 1, not 0
...
#                                 numpy.ceil(half_size), rel_yc_array)
      ...
      # --- Create columns      
      x_col  = SExCatalog.Column("x", format="% 5.1f", 
                                       comment="stamp x lower-left corner", data=x_array)
      y_col  = SExCatalog.Column("y", format="% 5.1f",
                                       comment="stamp y lower-left corner", data=y_array)

      xc_col = SExCatalog.Column("xc", format="% 6.3f", 
                                        comment="zero-index x centroid", data=xc_array)
      yc_col = SExCatalog.Column("yc", format="% 6.3f", 
                                        comment="zero-index y centroid", data=yc_array)

      rel_xc_col = SExCatalog.Column("rel_xc", format="% 5.1f", 
                                               comment="relative centroid x pixel in stamp", 
                                               data=rel_xc_array)
      rel_yc_col = SExCatalog.Column("rel_yc", format="% 5.1f", 
                                               comment="relative centroid y pixel in stamp", 
                                               data=rel_yc_array)

      # Append columns
      catalog.append_col(xc_col)
      catalog.append_col(yc_col)
      catalog.append_col(x_col)
      catalog.append_col(y_col)
      catalog.append_col(rel_xc_col)
      catalog.append_col(rel_yc_col)

#      # Mark centroids in check .FITS file if requested
#      centroids = zip(yc_array, xc_array) # using numpy convention 
#      self._mark_centroids(se_output_check_filepath, centroids, job, worker)

#      # Mark stamps in check .FITS file if requested
#      coords = zip(x_array, y_array) # using numpy convention
#      self._mark_stamps(se_output_check_filepath, coords, file_type, job, worker)

      # --- Save changes
      catalog.save(write_col_info=True, write_header=True)
...
#                                              mark_stamp_pix_value, marking_shape="cross")
   ...
   def _handle_splitted_objects(self, se_catalog_filepath, se_output_check_filepath,
      ...
      # Object coordinates
      x_array = catalog.get_col_data("x").astype(int)         
      y_array = catalog.get_col_data("y").astype(int)         
      coords = zip(y_array, x_array)    # (row,col) numpy convention

      xc_array = catalog.get_col_data("xc")
      yc_array = catalog.get_col_data("yc")
      centroid_coords = zip(yc_array, xc_array) # (row,col) numpy convention

      rel_xc_array = catalog.get_col_data("rel_xc")         
      rel_yc_array = catalog.get_col_data("rel_yc")  
   ...
   def _filter_flagged_entries(self, se_catalog_filepath, file_type, se_output_check_filepath,  
      ...
      obj_array = catalog.get_col_data(worker.config.get_as_string(
                                                         "NUMBER_PARAM", "PARAMETER_MAPPING"))
      flag_array = catalog.get_col_data(worker.config.get_as_string(
                                                         "FLAGS_PARAM", "PARAMETER_MAPPING"))
      obj_flux  = catalog.get_col_data(worker.config.get_as_string(
                                                         "FLUX_PARAM", "PARAMETER_MAPPING"))     

      rows = catalog.get_col_data("y").astype(int)         
      cols = catalog.get_col_data("x").astype(int) 
   ...
   def _create_stamp_mosaic(self, image_filepath, 
      ...
      # --- Catalog information
      x_coords = catalog.get_col_data("x").astype(int) 
      y_coords = catalog.get_col_data("y").astype(int) 
