...
class PseHelper(Helper):
   ...
   def show_config_summary(self, master):
      ...
      if master.logging_enabled():
         ...
         # TODO: also display the versions of the pipeline modules used by gfit
#         try:
#            multifit = __import__('multifit', globals(), locals(), [], -1)
#            model_fitter = multifit.ModelFitter()
##            master.logger.log_info_p("- multifit {0}\n".format(model_fitter.version))  
#         except Exception as detail:
#            master.logger.log_error_p("- multifit could not be imported: {0}\n".format(detail)) 
...
#         master.logger.log_info_p("Base configuration: {0}".format(gfit_config_path))
...
#         master.logger.log_info_p("Multifit modules:")
...
#         master.logger.log_info_p("PSF dataset              : {0}\n".format(psf_dataset_dir))
   ...
   # -----------------------------------------------------------------------------------------------
   def _get_catalog_nb_entries(self, catalog_filepath):
