...
class FittingModel(BaseFittingModel):
   ...
   def __init__(self, config_dir='./config/models', config_filename="gschsersic.cfg"):
      """! 
         Construct a @c gscshsersic FittingModel object 
         @param config_dir configuration directory of the method
         @param config_filename configuration directory of the method
      """  

      BaseFittingModel.__init__(self, name="gscshsersic", 
   ...
   def out_of_bound_handler(self):
      ...
      """
      #print "***MFIT: gscshsersic, getting  out_of_bound_handler..."
   ...
   def numerical_error_handler(self):
      ...
      """
      #print "***MFIT: gscshsersic, getting  numerical_error_handler..."
   ...
   #      status = 0  # continue fitting
      ...
      #print "***MFIT: in gscshsersic out_of_bound handler", iiter, param, param_value, bound_index
  ...
  # -----------------------------------------------------------------------------------------------
   def gscshsersic(self, model, stamp_size, subscale, flux, dn, dre, e1, e2, xc, yc):
      """!
      ...
      #print flux, dn, dre, e1, e2, xc, yc, "subscale:",  subscale, "stamp_size:", stamp_size
      ...
      try:
         ...
         gal = galsim.Sersic(n=dn, half_light_radius=dre, flux=flux)
...
#         gal.draw(gal_image, dx=pixel_scale)
         ...
         # --- Abort the fit
         print("*** gscshsersic model *** An error occurred: {0}".format(detail))
         #self.helper.print_error("gscshsersic model: an error occurred: {0}".format(sys.exc_info()[1]))
