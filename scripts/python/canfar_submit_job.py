    ...
    def set_options_base(self):
        ...
        if self._params["test"] == 1:
            ...
            if self._params["exclusive"]:
                tile_ID = self._params["exclusive"]
            else:
                # Ony used for debug file name
                tile_ID = self._params["file_IDs"]

            cwd = os.getcwd()
            if self._params['debug_out']:
                opt["debut_out"] = f"--debug_out {self._params['debug_out']}"
            else:
                opt["debut_out"] = f"--debug_out {cwd}/debug/debug_{tile_ID}.txt"
