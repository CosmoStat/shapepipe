...
from wgfit_shape import *         # galaxy shape estimation
from wgfit_plot import *          # plotter 
from wgfit_helper import *        # helper utility functions

# -------------------------------------------------------------------------------------------------
class WGfitJobProcessor(MpfxJobProcessor):
   ...
   def __init__(self, master):
      ...
      self._shape_estimator = WGfitShapeEstimator(master) 
      self._plotter = WGfitPlotter()    # plotter
      self._helper = WGfitHelper()      # helper
   ...
   def create_dataset(self, master, dataset_name, dataset_type, 
      ...
      """! 
         ...
         @param dataset_dir_list [optional] a list of specific directories to search under base directory
   ...
   def create_jobs(self, master):
      ...
      # --- Check that the jobs constains all necessary data 
      is_valid = True
      ...
      if len(job_list) > 0:
         ...
         if master.logging_enabled() and not is_valid:
            if len(job.galaxy_se_filepath_dico) == 0:
               ...
               master.logger.log_error_p("{0} - Failed to find input "\
            ...
            if len(job.psf_filepath_dico) == 0:
               ...
               master.logger.log_error_p("{0} - Failed to find input PSF images {1}".format(
                                             master, file_patterns))
   ...
   def create_job(self, master, dataset, *args):
      """! 
         Factory method for creating a WGfitJob object. 
   ...
   def preprocess_job(self, job, worker):
      """!
         ...
         @param job an object of class WGfitJob to process
         @param worker instance of the worker process

         @return an object of class WGfitJobResult containing the data of the processed job
      ...
      """
      pass  # we no nothing here in wgfit

   # -----------------------------------------------------------------------------------------------
   def process_job(self, job, worker):
      """! 
         Process a job of class WGfitJob and return the corresponding results in the form of a 
         WGfitJobResult object to the Master.       

         @param job object of class WGfitJob with processed data
      ...
      #     (see description of class: WGfitJob below).  
         ...
         for file_type in file_types: 
            ...
            if  job.dataset.is_galaxy_image(worker, filepath):
               ...
               try:
                  ...
                  # --- Create a ShapeMeasurementRequest object to process by the ShapeEstimator 
#                  cat_file_extension = worker.config.get_as_string("CATALOG_EXTENSION", 
#                                                                   "IMAGE_PROPERTIES.GALAXY")
                  ...
                  if not shape_request is None:
                     ...
                     object_per_type_dico[file_type] = self.shape_estimator.measure_galaxy_shapes(
                                                                                 shape_request,
                                                                                 job, worker)
               except WGfitShapeEstimator.ShapeMeasurementError as detail:
                  if worker.logging_enabled():
      ...
      # --- Create a WGfitJobResult object with the results from all image types
      return WGfitJobResult(job, object_per_type_dico, worker)

   # -----------------------------------------------------------------------------------------------
   def postprocess_job(self, job_result, worker):
      """! 
         Invoked by the Worker to perform some optional post-processing on the job.
         @param job_result object of class WGfitJobResult with processed data
         @param worker instance of the worker process
   ...
   def process_job_result(self, job_result, master):
      ...
      for file_type in object_per_type_dico.keys():
         ...
         model_dico  = object_per_type_dico[file_type]["model"]    # infor about the model
         ...
         self._create_result_catalog(result_dico, error_dico, filter_dico, info_dico, job, master)

         if master.config.get_as_boolean("DUMP_COLLECTED_DATA", "DATA_COLLECTION"):

            # --- Create a catalog that includes all collected data for this job
            output_result_data_directory = os.path.join(master.result_output_dir, 
                                                        job.get_branch_tree())
            output_error_data_directory  = os.path.join(master.error_output_dir, 
                                                        job.get_branch_tree())

            data_file_prefix_main, data_file_prefix_main_ext = os.path.splitext(file_type)
            output_result_data_filename = "data_{0}-{1:03}.txt".format(data_file_prefix_main, 
                                                                       job.img_no)
            output_error_data_filename = "errors_{0}_{1:03}.txt".format(
                                     "_".join(job.branch_dirs), job.img_no)
            output_filtered_data_filename = "filtered_{0}-{1:03}.txt".format(
                                     "_".join(job.branch_dirs), job.img_no)

            self._dump_all_collected_data(result_dico, error_dico, filter_dico, model_dico, 
                                          output_result_data_directory, output_result_data_filename,  
                                          job, master)

            if len(error_dico["GAL_id"]) > 0:
               self._dump_all_collected_data(error_dico, error_dico, filter_dico, model_dico,
                                             output_error_data_directory, 
                                             output_error_data_filename, job, master)

            if len(filter_dico["GAL_id"]) > 0:
               self._dump_all_collected_data(filter_dico, error_dico, filter_dico, model_dico,
                                             output_error_data_directory, 
                                             output_filtered_data_filename, job, master)

         # --- Summary plots at job level
         if master.config.get_as_boolean("PLOT_COLLECTED_DATA", "DATA_COLLECTION"):
            self.plotter.create_image_plots(object_per_type_dico, file_type, job, master)         
...
#      #print "#############", self.job_result_dico
      for branch_tree in self._get_selected_branches(master):
   ...
   def _create_result_catalog(self, result_dico, error_dico, filter_dico, info_dico, job, master):
      ...
      # --- Create a submission result catalog, marking failed ellipticity values 
      #     with a constant >= 10
      failed_marking_value = master.config.get_as_float("FAILED_ELLIPTICITY_VALUE", 
                                                        "SHAPE_MEASUREMENT")

      output_filename = "results_{0}-{1:03}.txt".format("_".join(job.branch_dirs), job.img_no)
   ...
   def _dump_all_collected_data(self, result_dico, error_dico, filter_dico, model_dico, 
                                      output_directory, output_filename, job, master):
...
#      result_dico_copy = self.helper._remove_failed_fits_from_result_dico(result_dico_copy, master)
      ...
      failed_marking_value = master.config.get_as_float("FAILED_ELLIPTICITY_VALUE", 
                                                        "SHAPE_MEASUREMENT")
      if failed_marking_value in result_dico_copy["e1"] or failed_marking_value in result_dico_copy["e2"]:

         # --- Get filtered elipticity values
         for (ID, e1) in zip(filter_dico["GAL_id"], filter_dico["e1"]):
            result_dico_copy["e1"][result_dico_copy["GAL_id"].index(ID)] = e1       
         for (ID, e2) in zip(filter_dico["GAL_id"], filter_dico["e2"]):
            result_dico_copy["e2"][result_dico_copy["GAL_id"].index(ID)] = e2 

         # --- Get failed elipticity values
...
# -------------------------------------------------------------------------------------------------
class WGfitJob(MpfxJob):      

   """! 
       A WGfit job. Based on mpfg.Job and mpfx.MpfxJob.  
   """
   ...
   def __init__(self, master, dataset, img_path_dico, *args):
      """!   
         ...
         @return an initialized WGfitJob object
      """
...
#      return self._psf_stamp_size
   ...
   def galaxy_se_filepath_dico(self, galaxy_se_filepath_dico):
      """! 
         ...
         @param galaxy_se_filepath_dico filepaths dictionary containing for each file_type, image number and epoch the
         absolute path of SExtractor matching files
      """
   ...
   def star_se_filepath_dico(self, star_se_filepath_dico):
      """! 
         ...
         by this job
         @param star_se_filepath_dico dictionary containing for each file_type, image number and epoch the
                                     absolute path of SExtractor matching files
      """
   ...
   def psf_filepath_dico(self, psf_filepath_dico):
      """! 
         ...
         this job
         @param psf_filepath_dico dictionary containing for each file_type, image number and epoch the
                            absolute path of PSF matching files
      """
...
# -------------------------------------------------------------------------------------------------
class WGfitJobResult(MpfxJobResult):      
   ...
   def __init__(self, job, result, worker):   
      """! Construct a WGfitJobResult job result object """
