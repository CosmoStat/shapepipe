...
# The members this rule selects, named and resolved through the catalogue
# exp_persist packs by, so the glob has one definition. They are always there to
# find: persist_exp packs this product for every exposure whatever `persist_exp:`
# says (its ALWAYS), and fails the pack rather than writing a manifest without it.
MEMBER_PRODUCT = persist_exp.ALWAYS
MEMBER_PATTERN = persist_exp.resolve(MEMBER_PRODUCT)

# Only the PREFIX of a member name is ours to know; the rest is the unit id and
# the CCD index, which is what makes the set a set of indices. Derived from the
# pattern so it cannot drift from it.
PSF_PREFIX = MEMBER_PATTERN.split("-", 1)[0]


def valid_ccds(manifest, exp):
    """The CCD indices with a PSF model, read off an ``exp_persist`` manifest.

    BY PRODUCT NAME, OR FAILING THAT BY FILE NAME — merge_star_cat.is_member()'s
    test, and for its reasons: persist_exp labels every member with the product
    it came from, but a tar packed before that field existed carries no label,
    and a keep list written as a raw glob labels its members with the glob.

    No precondition on the keep list. exp_persist packs this product for every
    exposure whatever `persist_exp:` says, so an empty answer here means the
    exposure genuinely lost every CCD, not that the config unpacked them.
    """
    body = json.loads(Path(manifest).read_text())
    ccds = set()
    for entry in body.get("files") or []:
        name = entry.get("name", "")
        if not (entry.get("product") == MEMBER_PRODUCT
                or fnmatch(name, MEMBER_PATTERN)):
            continue
