...
and `clean_exposure` does not wait on them; `coverage.enabled` has no map to
build.
...
workflow/
  ...
  rules/
    ...
    exposure.smk         per-exposure: get_images, split, psf, persist, footprint, defect_map (no temp()); campaign star_cat_merge, defect_map_merge
