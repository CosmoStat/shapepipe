...
# `tile_detection` is `sextractor` (the tile is detected with SExtractor) or
# `unions_catalogue` (the UNIONS per-tile catalogue at `inputs.catalogues` is
# fetched and converted in place, and its object ID rides into the final
# catalogue as TILE_UNIQUE_ID; no segmentation map, so not with ngmix's uberseg).
