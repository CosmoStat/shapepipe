"""! 
   @package mpfcs82.mpfcs82_job Job Management
   @author Marc Gentile
   @file mpfcs82_job.py
   Job Management
""" 
...
from mpfcs82_data import *           # dataset management

# --------------------------------------------------------------------------------------------------
class MpfCS82JobProcessor(MpfxJobProcessor):
   ...
   def process_job(self, job, worker):
      ...
      return MpfCS82JobResult(worker, job, data_dico)  # store the job results
   ...
   def create_job(self, master, dataset, *args):
      """! 
         ...
         @return a new MpfCS82Job job object
         @see Job, MpfCS82Job
      """

      return MpfCS82Job(master, dataset, *args)  # new MpfCS82Job instance, based on parent class mpfx.MpfxJob
   ...
   def create_dataset(self, master, dataset_name, dataset_type, 
      ...
      return MpfCS82Dataset(master, dataset_name, 
                                    dataset_base_dir, dataset_dir_list, dataset_recurse)
   ...
   def get_job_results(self, **kwargs):
      """! 
         Return the list of MpfCS82JobResult objects for a specified branch, observation type, 
         data_type and image number. These parameters must be specified using the 
         "branch=", "img_no=" and "epoch=" keywords
         @return list of MpfCS82JobResult objects
         @code
            Examples
         ...
         @endcode
         @note MpfCS82JobResult objects can also be directly obtained from the 
               mpfcs82.mpfcs82_job.MpfCS82JobProcessor.job_result_dico dictionary
      """
...
# -------------------------------------------------------------------------------------------------
class MpfCS82Job(MpfxJob):      

   """! 
       Represents a MpfCS82 job 
   """
   ...
   def __init__(self, master, dataset, img_path_dico, *args):
      """! 
         ...
         @return an initialized MpfCS82Job object 
      """
   ...
   def get_stamp_size(self):
      ...
      if self.branch != "multiepoch" and self.branch != "full":
         return {"ground":48, "space":96}.get(self.obs_type, -1)
...
# -------------------------------------------------------------------------------------------------
class MpfCS82JobResult(MpfxJobResult):      
   ...
   def __init__(self, worker, job, result):   
      """! Construct a MpfCS82JobResult job result object """

      MpfxJobResult.__init__(self, worker, job, result)


# --------------------------------------------------------------------------------------------------
class MpfCS82MultiEpochJobProcessor(MpfxMultiEpochJobProcessor):
   ...
   """! 
      ...
      Based on mpf.MpfxMultiEpochJobProcessor.   
   """

   def __init__(self, master):
      """ MpfxMultiEpochJobProcessorProcessor default constructor """
   ...
   def create_dataset(self, master, dataset_name, dataset_type, 
      ...
      return MpfCS82MultiEpochDataset(master, dataset_name, 
                                      dataset_base_dir, dataset_dir_list, dataset_recurse)
   ...
   def create_job(self, master, dataset, *args):
      """! 
         ...
         @param args a list of arguments chosen by the caller and whose nature depends on the 
                data necessary to process the job
         @return a new MpfCS82MultiEpochJobjob object
         @see Job, MpfCS82MultiEpochJob
      """

      # --- Create a MpfxMultiEpochJob instance, based on parent class mpfx.MpfxJob
      return MpfCS82MultiEpochJob(master, dataset, *args)  
   ...
   def process_job(self, job, worker):
      ...
      data_dico = {"result": "result from MpfCS82MultiEpochJob: {0}".format(job.name)} 

      return MpfCS82MultiEpochJobResult(worker, job, data_dico)  # store the job results


# -------------------------------------------------------------------------------------------------
class MpfCS82MultiEpochJob(MpfxMultiEpochJob):      

   """! 
       Represents a MpfxMultiEpochJob job for holding a group of images with multiple exposures, 
       with related catalogs. 
   """
   ...
   def __init__(self, master, dataset, img_path_dico, *args):
      """! 
         ...
         @return an initialized MpfCS82MultiEpochJob object 
      """
...
# -------------------------------------------------------------------------------------------------
class MpfCS82MultiEpochJobResult(MpfxMultiEpochJobResult):      
   ...
   def __init__(self, worker, job, result):   
      """! Construct a MpfCS82MultiEpochJobResult job result object """

      MpfxMultiEpochJobResult.__init__(self, worker, job, result)
