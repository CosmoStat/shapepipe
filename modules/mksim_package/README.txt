...
Suimulation Tool for Shear Measurement
...
- GalSim v1.2.x: galaxy image simulation toolkit (https://github.com/GalSim-developers/GalSim)

Installation:
-------------

- Download and unpack the file mksim-0.5.0.tar.gz in the directory of your
  choice
- mpfcs82 is a pure Python module and can then be installed using the standard
  procedure: 

     1. create a console or terminal session
     2. cd to the directory where mksim-0.5.0.tar.gz was unpacked
...
The general syntax is: 
  ...
  SMP version: mksim_SMP.py [options]  
  MPI version: mksim_MPI.py [options]  

by default, mksim will look for a configuration file named mksim.cfg in the
./mksim/config directory. The location of this file can be changed using the -c and 
-d options (see below).
...
To run mksim_MPI.py, one has to use the mpirun executable as:

   - <code>mpirun -np N mksim_MPI.py [options]</code>
...
For example: 'mpirun -np 6 mksim_MPI -d mydir -c myconfig.cfg' will run Quadg3
...
Before running mksim, edit the configuration file to set the BASE_INPUT_DIR value, which
