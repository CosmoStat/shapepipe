...
"""! 
   mks_MPI.py - Multiprocessing with MPI - MKSIM
"""


# -------------------------------------------------------------------------------------------------
class MksMasterMPI(MpfxMasterMPI):
   ...
   def create_job_processor(self):
      ...
      """
      print "### MKS create_job_processor()"

      return jobp.MksJobProcessor(self)


# -------------------------------------------------------------------------------------------------
class MksWorkerMPI(WorkerMPI):      
...
if __name__ == '__main__':
   ...
   if rank == 0:
      ...
      try:
         ...
         if master is None:
            ...
            try:
               ...
               MksHelper.print_error("The master process ended unexpectedly ({0})".format(detail))
            finally:
               master.shutdown()
               sys.exit(0)
      ...
      try:
         ...
         # --- Worker process
         worker = MksWorkerMPI(args, comm, rank)  
         if worker is None:
            sys.exit(2)
         else:
            try:
               ...
               MksHelper.print_error("The worker process {0} ended unexpectedly ({1})".format(
                                                                                    worker.name, 
                                                                                    detail))
            finally:
               sys.exit(0)
