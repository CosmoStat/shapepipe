...
class MasterSMP(Master):
   ...
   def run(self):
      ...
      try:
         ...
         # --- Run workers: the should all b until 
         self.run_workers()

         ###print "waiting a few secs", time.sleep(10)
...
class WorkerSMP(Worker):
   ...
   def run(self, input_queue, output_queue, start_event, shared_dico):
      ...
      try:
         ...
         ###print "WorkerSMP run()", self.name, "input queue empty ?", input_queue.empty(), "event set ?", start_event.is_set()
         

         # Wait until the queue gets populated by the main thread
         start_event.wait()

         ###print "WorkerSMP", self.name, "running, input queue empty ?", input_queue.empty(), "event set ?", start_event.is_set()
         ...
         ###print "WorkerSMP", self.name, "input queue empty ?", input_queue.empty(), start_event.is_set()
         ...
         ###print "WorkerSMP", self.name, "Terminating..."
         ...
         print "WorkerSMP", self.name, "Queue empty Exception"
