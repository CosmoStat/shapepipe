...
class PackageRunner(object):
    ...
    def _exec_code(self):
        ...
        This method executes the command script: ngmix_script.

        """

        option_dict = self._worker.config.get_section_data('NGMIX_OPTION')

        r = ngmix_script(gal_cat_path= self._fnames['input_filepath'][0],
                         psf_cat_path= self._fnames['input_filepath'][1],
                         output_dir= self._worker.result_output_dir,
                         option_dict= option_dict)
        r.process()

    def _get_option_dict(self):
        """
        """

        option_dict = {}

        option_dict['TYPES'] = self._worker.config.get_as_list('TYPES', 'NGMIX_OPTION')
        option_dict['FIXNOISE'] = self._worker.config.get_as_boolean('FIXNOISE', 'NGMIX_OPTION')
