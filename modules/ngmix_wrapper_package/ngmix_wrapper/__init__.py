...
The Python program files are located in the ./ngmix_wrapper/ directory and the
default configuration file
...
- scatalog v2.0.2: catalog management

- ngmix : metacalibration software (https://github.com/esheldon/ngmix)

Installation

- Download and unpack the file ngmix_wrapper-1.0.0.tar.gz in the directory of
  your choice
- ngmix_wrapper is a pure Python module and can then be installed using the
  standard procedure:

  <ol>
   <li> create a console or terminal session </li>
   <li> cd to the directory where ngmix_wrapper-1.0.0.tar.gz was unpacked </li>
...
The general syntax is:
  ...
  SMP version: ngmix_wrapper_SMP.py [options]
  MPI version: ngmix_wrapper_MPI.py [options]

by default, ngmix_wrapper will look for a configuration file named config.cfg in
the ./ngmix_wrapper/config directory. The location of this file can be changed
...
To run ngmix_wrapper_MPI.py, one has to use the mpirun executable as:

   - <code>mpirun -np N ngmix_wrapper_MPI.py [options]</code>
...
For example: 'mpirun -np 6 ngmix_wrapper_MPI -d mydir -c myconfig.cfg' will run
...
Notes
-----
Before running ngmix_wrapper, edit the configuration file to set the
