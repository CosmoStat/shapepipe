...
The Python program files are located in the ./MakeVignet/ directory and the
default configuration file
...
- Download and unpack the file MakeVignet-1.0.0.tar.gz in the directory of
  your choice
- MakeVignet is a pure Python module and can then be installed using the
  standard procedure:

  <ol>
   <li> create a console or terminal session </li>
   <li> cd to the directory where MakeVignet-1.0.0.tar.gz was unpacked </li>
...
The general syntax is:
  ...
  SMP version: MakeVignet_SMP.py [options]
  MPI version: MakeVignet_MPI.py [options]

by default, MakeVignet will look for a configuration file named config.cfg in
the ./MakeVignet/config directory. The location of this file can be changed
...
To run MakeVignet_MPI.py, one has to use the mpirun executable as:

   - <code>mpirun -np N MakeVignet_MPI.py [options]</code>
...
For example: 'mpirun -np 6 MakeVignet_MPI -d mydir -c myconfig.cfg' will run
...
Notes
-----
Before running MakeVignet, edit the configuration file to set the
