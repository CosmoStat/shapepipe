...
- Download and unpack the file MakeVignet-1.0.0.tar.gz in the directory of your
  choice
- mpfcs82 is a pure Python module and can then be installed using the standard
  procedure:

     1. create a console or terminal session
     2. cd to the directory where MakeVignet-1.0.0.tar.gz was unpacked
...
by default, MakeVignet will look for a configuration file named MakeVignet.cfg in the
./MakeVignet/config directory. The location of this file can be changed using the -c and
-d options (see below).
...
For example: 'mpirun -np 6 MakeVignet_MPI -d mydir -c myconfig.cfg' will run Quadg3
...
Before running MakeVignet, edit the configuration file to set the BASE_INPUT_DIR value, which
