...
Sprite::Sprite(int upfact,fltarray *data,int upfact_0,mr_opt mod_opt,mr_opt sr_opt,int nb_iter,double r,double sig_gfit,int mod_denoising_type,double nsig,string output_dir,string output_file_name,string res_file_name,int noise_est_meth,int min_id,Bool eq_flux,Bool eq_noise,int win,double tol,bool Verbose,char * shift_filename,double spectral_rad,Bool fft_en,int ref_im_ind,Bool pos_cons,Bool deconv_en,Ifloat*conv_mask,int lanc_rad)
{
  int i,j,k;
  this->upfact = upfact;
  this->data = data;
	...
	for (j=0;j<Ny;j++) 
	  {
	    ...
	    if (eq_noise == False)
	      w = noise_sig_map(i,j,k);
	    op_mat_output(i,j,k)=sr_dec(i,j)*flux[k]/w;
	...
	for (j=0;j<Ny;j++) 
	  {
	    ...
	    if (eq_noise == False)
	      w = noise_sig_map(i,j,k);
	    output(i,j,k)=sr_dec(i,j)*flux[k]/w;
	  }
...
void Sprite::sprite_transp_op_mat(double *input,double *output)
{
  ...
  for (k=0;k<Nz;k++)
	...
	for (j=0;j<Ny*upfact;j++) 
	  gradient(i,j)=gradient(i,j)+flux[k]*hr_temp_filt(i,j);
    }      
...
void Sprite::sprite_transp_op_mat(fltarray input,Ifloat &output)
{
  ...
  for (k=0;k<Nz;k++)
	...
	for (j=0;j<2*lanc_rad+1;j++) lanc_ker_temp(i,j) = lanczos_ker(i,j,k);
      ...
      for (i=0;i<Nx*upfact;i++)
	for (j=0;j<Ny*upfact;j++) 
	  output(i,j)=output(i,j)+flux[k]*hr_temp_filt(i,j);
    }      
...

{
  ...
  for (l=0;l<nb_rw;l++)
      ...
      for (m=0;m<nb_iter;m++)
	{
	  ...
	  for (i=0;i<Nx;i++)
	    for (j=0;j<Ny;j++) 
	      for (k=0;k<Nz;k++) resi(i,j,k) = data_nrm(i,j,k)-op_mat_output(i,j,k);
	...
	for (j=0;j<Ny*upfact;j++)
	  { 
	    iter_diff += abs(delta_old(i,j)-delta_old_2(i,j)); 
	    delta_old_l1+=abs(delta_old_2(i,j));
	  }
      ptr_e = m%cv_win;
      ...
      if (m>=cv_win)
	{
      ...
      for (i=0;i<Nx*upfact;i++)
	for (j=0;j<Ny*upfact;j++) 
	  for (k=0;k<nb_band-1;k++) weights(i,j,k) = 1/(1+abs(coeff_thresh(i,j,k))/(3*wvl_noise_map(i,j,k)));
      if(Verbose==True)
