...
class MR_UpSamp
{
  ...
  void mod_gen(int ref_im_ind,int denoising_type,float zoom=-1,double nsig=3,bool thresh_type=0);
    ...
    void op_mat(double *input=NULL,double *output=NULL);
    void op_mat(Ifloat input,fltarray &output); // For unitary tests
    void transp_op_mat(double *input=NULL,double *output=NULL);
    void transp_op_mat(fltarray input,Ifloat &output); // For unitary tests
    void global_op_mat(double *input,double *output);
