...
static void usage(char *argv[])
{
  ...
  fprintf(OUTMAN, "        [-d]\n");
  fprintf(OUTMAN, "             If set to the sparsity constraint is disabled.\n");
...
static void filtinit(int argc, char *argv[])
{
  ...
  /* get options */
  while ((c = GetOpt(argc,argv, (char *) "t:T:c:n:s:i:r:R:w:e:I:m:C:a:S:V:dWFNvzZ:")) != -1) 
    { 
      switch (c) 
        {
	case 't':
...
int main(int argc, char *argv[]) 
{
  ...
  mr_opt sr_opt = mr_opt_init(Nx*ResolUp,transform,Nbr_Plan,Nbr_Plan,NORM_L1,DEF_UNDER_FILTER,F_MALLAT_7_9,I_CONT,False);
  int siz = Nx*ResolUp;
  ...
  Sprite sprite_test(ResolUp,&Dat,D0,mod_opt,sr_opt,Max_Iter,r,sig,Mod_denoise_meth,N_Sigma,rep_path,output_file,resi_file,Noise_est_meth,0,eq_flux,eq_noise,cv_win,cv_tol,Verbose,shift_filename,Sparse_en,dist_weights_filename);
  Bool Wr_en=True;
