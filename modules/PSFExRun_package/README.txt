...
- Download and unpack the file PSFEx-1.0.0.tar.gz in the directory of your
  choice
- mpfcs82 is a pure Python module and can then be installed using the standard
  procedure:

     1. create a console or terminal session
     2. cd to the directory where PSFEx-1.0.0.tar.gz was unpacked
...
by default, PSFEx will look for a configuration file named PSFEx.cfg in the
./PSFEx/config directory. The location of this file can be changed using the -c and
-d options (see below).
...
For example: 'mpirun -np 6 PSFEx_MPI -d mydir -c myconfig.cfg' will run Quadg3
...
Before running PSFEx, edit the configuration file to set the BASE_INPUT_DIR value, which
