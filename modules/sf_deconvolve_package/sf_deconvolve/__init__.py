...
@mainpage Multiprocessing Framework - Parallel sf_deconvolve execution
...
The Python program files are located in the ./sf_deconvolve/ directory and the
default configuration file
...
- Download and unpack the file sf_deconvolve-1.0.0.tar.gz in the directory of
  your choice
- sf_deconvolve is a pure Python module and can then be installed using the
  standard procedure:

  <ol>
   <li> create a console or terminal session </li>
   <li> cd to the directory where sf_deconvolve-1.0.0.tar.gz was unpacked </li>
...
The general syntax is:
  ...
  SMP version: sf_deconvolve_SMP.py [options]
  MPI version: sf_deconvolve_MPI.py [options]

by default, sf_deconvolve will look for a configuration file named config.cfg in
the ./sf_deconvolve/config directory. The location of this file can be changed
...
To run sf_deconvolve_MPI.py, one has to use the mpirun executable as:

   - <code>mpirun -np N sf_deconvolve_MPI.py [options]</code>
...
For example: 'mpirun -np 6 sf_deconvolve_MPI -d mydir -c myconfig.cfg' will run
...
Notes
-----
Before running sf_deconvolve, edit the configuration file to set the
