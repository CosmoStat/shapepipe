parallel sf_deconvolve v1.0.0
=============================

Parallel execution of sf_deconvolve
...
- Download and unpack the file sf_deconvolve-1.0.0.tar.gz in the directory of your
  choice
- mpfcs82 is a pure Python module and can then be installed using the standard
  procedure:

     1. create a console or terminal session
     2. cd to the directory where sf_deconvolve-1.0.0.tar.gz was unpacked
...
by default, sf_deconvolve will look for a configuration file named sf_deconvolve.cfg in the
./sf_deconvolve/config directory. The location of this file can be changed using the -c and
-d options (see below).
...
For example: 'mpirun -np 6 sf_deconvolve_MPI -d mydir -c myconfig.cfg' will run Quadg3
...
Before running sf_deconvolve, edit the configuration file to set the BASE_INPUT_DIR value, which
