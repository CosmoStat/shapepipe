...
The Python program files are located in the ./shapelens/ directory and the
default configuration file
...
- shapelens : shape measurement software (https://github.com/pmelchior/shapelens).

Installation

- Download and unpack the file shapelens-1.0.0.tar.gz in the directory of
  your choice
- shapelens is a pure Python module and can then be installed using the
  standard procedure:

  <ol>
   <li> create a console or terminal session </li>
   <li> cd to the directory where shapelens-1.0.0.tar.gz was unpacked </li>
...
The general syntax is:
  ...
  SMP version: shapelens_SMP.py [options]
  MPI version: shapelens_MPI.py [options]

by default, shapelens will look for a configuration file named config.cfg in
the ./shapelens/config directory. The location of this file can be changed
...
To run shapelens_MPI.py, one has to use the mpirun executable as:

   - <code>mpirun -np N shapelens_MPI.py [options]</code>
...
For example: 'mpirun -np 6 shapelens_MPI -d mydir -c myconfig.cfg' will run
...
Notes
-----
Before running shapelens, edit the configuration file to set the
