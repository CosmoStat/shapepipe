PSFExInterpolation v1.0.0
===================

Read a PSFEx model from a .psf file (PSFEx output) and compute the PSF model at the desired
...
- psfex-rec program, found in the ``psfex" GitHub repo: https://github.com/esheldon/psfex

Installation:
-------------

- Download and unpack the file PSFExInterpolation-1.0.0.tar.gz in the directory of your
  choice
- mpfcs82 is a pure Python module and can then be installed using the standard
  procedure:

     1. create a console or terminal session
     2. cd to the directory where PSFExInterpolation-1.0.0.tar.gz was unpacked
...
by default, PSFExInterpolation will look for a configuration file named PSFExInterpolation.cfg in the
./PSFExInterpolation/config directory. The location of this file can be changed using the -c and
-d options (see below).
...
For example: 'mpirun -np 6 PSFExInterpolation_MPI -d mydir -c myconfig.cfg' will run Quadg3
...
Before running PSFExInterpolation, edit the configuration file to set the BASE_INPUT_DIR value, which
