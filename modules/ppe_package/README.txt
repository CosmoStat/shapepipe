...
Parallel execution of multiple PSFEx processes
...
- An installation of the PSFEx tool (http://www.astromatic.net/software/psfex) is also
  required.
...
- mpfcs82 is a pure Python module and can then be installed using the standard
  procedure: 
     ...
     2. cd to the directory where ppe-1.0.0.tar.gz was unpacked
...
./ppe/config directory. The location of this file can be changed using the -c and 
-d options (see below).
...
Before running ppe, edit the configuration file to set the BASE_INPUT_DIR value, which
