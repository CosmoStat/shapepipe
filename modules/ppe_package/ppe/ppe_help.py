...
from ppe_version import __version__  # version information
...
#     print "OPEN:", "catalog:", catalog, "nb_rows:", catalog.get_nb_rows(), "format:", catalog.format
   ...
   def collect_catalog_data(self, object_per_type_dico, job, worker):
      ...
      for file_type in object_per_type_dico.keys():
         ...
         pe_run_dico = object_per_type_dico[file_type]["pe_run_dico"]
         ...
         if pe_catalog.get_nb_rows() > 0:
            ...
            flux_data = pe_catalog.get_named_col_data(flux_colname)
            fluxerr_data = pe_catalog.get_named_col_data(fluxerr_colname)
            data_dico[file_type]["SNR"] = numpy.nan_to_num(flux_data / fluxerr_data)

            # --- Allowed PSFEx column data
            for col_name in pe_catalog.get_col_names():
               if not col_name in forbidden_col_names:
                  data_dico[file_type][col_name] = pe_catalog.get_named_col_data(col_name)
   ...
   def compute_stats(self, object_per_type_dico, data_dico, job, master):
      """! Compute statistical operations on the PSFEx catalog data """
