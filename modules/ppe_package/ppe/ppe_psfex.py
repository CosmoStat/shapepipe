...
from ppe_counter import *           # counter class
...
class PSFExRunner(object):
   ...
   """!
      Run PSFEx and create a PSFEx catalog from an input image
   """
   ...
   def run_PSFEx(self, file_type, job, worker):
      ...
      try:
         ...
         # PSFEx configuration
         pe_config_filepath = self._get_psfex_config_filepath(image_filepath, file_type,
                                                            job, worker)

         if pe_config_filepath is not None:
            if worker.logging_enabled():
               worker.logger.log_info_p(
                   "{0} - /{1}/img-{2:03}-{3:1d} - {4} - "\
                   "Using PSFEx .psfex configuration file: {5} ...".format(
                     worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type,
                     pe_config_filepath))

         # --- Make a copy of the used .psfex file to the log directory for record
         shutil.copy(pe_config_filepath, worker.log_output_dir)

         # --- Target directory where to store files
         pe_output_path = os.path.join(worker.result_output_dir, job.get_branch_tree())

         # --- PSFEx catalog output filepath
         pe_output_cat_filename = self._get_output_pe_catalog_filename(image_filepath, job, config)

         # --- Output catalog type
         pe_output_cat_type = config.get_as_string("PE_OUTPUT_CATALOG_TYPE", "PSFEX")
         if pe_output_cat_type == "FITS_1.0":
            # --- If .fits format specified, force the output filename extension to .fits
            _, fileext = os.path.splitext(pe_output_cat_filename)
            if fileext in [".cat", ".txt"]:
               pe_output_cat_filename = pe_output_cat_filename.replace(fileext, ".fits")
         elif pe_output_cat_type in ["ASCII_SKYCAT", "ASCII_VOTABLE", "FITS_LDAC"]:
            if worker.logging_enabled():
               worker.logger.log_warning_p(
                  ...
                  "Using ASCII_HEAD format...".format(
                   worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type))
               worker.logger.flush()
            pe_output_cat_type = "ASCII_HEAD"
         else:
            pe_output_cat_type = "ASCII_HEAD"

         pe_output_cat_filepath = os.path.abspath(os.path.join(
                                                          pe_output_path, pe_output_cat_filename))

         if worker.logging_enabled():
            worker.logger.log_info_p(
                "{0} - /{1}/img-{2:03}-{3:1d} - {4} - Generating PSFEx catalog {5} ...".format(
                worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type,
                pe_output_cat_filepath))
            worker.logger.flush()

         # --- PSFEx check .fits output filepath
         pe_output_check_filename = self._get_output_pe_check_filename(image_filepath, job, config)
         pe_output_check_filepath = os.path.abspath(os.path.join(pe_output_path,
                                                                 pe_output_check_filename))

         # --- PSFEx redir .txt output filepath
         pe_must_redir_output = config.get_as_boolean("PE_REDIR_OUTPUT", "PSFEX")
         pe_output_redir_filename = self._get_output_pe_redir_filename(image_filepath, job, config)
         pe_output_redir_filepath = os.path.abspath(os.path.join(pe_output_path,
                                                                 pe_output_redir_filename))

         # --- Check image type
         pe_check_image_type = config.get_as_string("PE_CHECKIMAGE_TYPE", "PSFEX")
         if pe_check_image_type != "NONE":
            if worker.logging_enabled():
               worker.logger.log_info_p(
         ...
         pe_exec_line = pe_exec_path + ' ' + image_filepath + ' -PSF_DIR ' + pe_output_path
         ...
         pe_object_dico["pe_output_check_filepath"] = pe_output_check_filepath
         pe_object_dico["pe_output_redir_filepath"] = pe_output_redir_filepath
         pe_object_dico["pe_check_image_type"] = pe_check_image_type
            ...
            if os.path.exists(pe_output_cat_filepath):
             worker.logger.log_info_p(
               ...
               worker.logger.log_error_p(
                 "{0} - An error occurred while generating PSFEx catalog: {1}".format(
                                                                              worker.name, job))
            worker.logger.flush()
         ...
         if worker.logging_enabled():
            worker.logger.log_error_p(
                "{0} - An error occurred while generating PSFEx catalog: {1} ({2})".format(
                                                                                worker.name,
                                                                                job,
                                                                                detail))
   ...
   # -----------------------------------------------------------------------------------------------
   def _get_psfex_config_filepath(self, image_file_path, file_type, job, worker):
      """!
         Find and return the .psfex PSFEx cofiguration filepath to use.
         - First look for a .psfex file having the same filename as that of the image file in the
           @c input directory.
         - If no file is found, look for a file with the name specified by the
           @c PSFEX.PE_DEFAULT_PSFEX_FILENAME key in the configuration file.
      ...
      # --- Look for a .psfex file that complies with the same filename as that of the image but
      #     with a -psfex extension
      ...
      psfex_file_candidate = os.path.join(search_path, file_name.replace(file_ext, ".psfex"))

      if self.helper.file_exists(psfex_file_candidate):
         # --- Found a .psfex file for this image
         psfex_filepath = psfex_file_candidate
      else:
         # --- Look for a .psfex file having the same file_type
         psfex_file_candidate = os.path.join(search_path, file_type.replace(file_ext, ".psfex"))

         if self.helper.file_exists(psfex_file_candidate):
            # --- Found .psfex file applicable to all images with the same filetype
            psfex_filepath = psfex_file_candidate
         else:
            # --- Looking for a default .psfex file somehwre in the base input directory tree
            default_psfex_filename = worker.config.get_as_string("PE_DEFAULT_PSFEX_FILENAME",
                                                               "PSFEX")
            ...
            if len(found_files) > 0:
               ...
               psfex_filepath = os.path.abspath(os.path.join(worker.base_input_dir, "default.psfex"))

               if worker.logging_enabled():
                  worker.logger.log_warning_p(
                      "{0} - /{1}/img-{2:03}-{3:1d} - {4} - "\
                      "Could not find PSFEx .psfex file {5}- Trying {6}".format(
                        worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type,
                        default_psfex_filename, psfex_filepath))

      return psfex_filepath


   # -----------------------------------------------------------------------------------------------
   def _get_output_pe_catalog_filename(self, image_filepath, job, config):
      """!
         Build the SE output catalog filename based on the image filepath
      """
      _, image_filename = os.path.split(image_filepath)
      pe_catalog_prefix, _ = os.path.splitext(image_filename)
      pe_catalog_filename_pattern = config.get_as_string("PE_OUTPUT_CATALOG_FILENAME", "PSFEX")
      return pe_catalog_filename_pattern.format(pe_catalog_prefix)

   # -----------------------------------------------------------------------------------------------
   def _get_output_pe_check_filename(self, image_filepath, job, config):
      """!
         Build the SE check .fits filename based on the image filepath
      """
      _, image_filename = os.path.split(image_filepath)
      pe_check_prefix, _ = os.path.splitext(image_filename)
      pe_check_filename_pattern = config.get_as_string("PE_OUTPUT_CHECK_FILENAME", "PSFEX")
      return pe_check_filename_pattern.format(pe_check_prefix)

   # -----------------------------------------------------------------------------------------------
   def _get_output_pe_redir_filename(self, image_filepath, job, config):
      """!
         Build the SE redir .txt filename based on the image filepath
      """
      _, image_filename = os.path.split(image_filepath)
      pe_redir_prefix, _ = os.path.splitext(image_filename)
      pe_redir_filename_pattern = config.get_as_string("PE_OUTPUT_REDIR_FILENAME", "PSFEX")
      return pe_redir_filename_pattern.format(pe_redir_prefix)



# -------------------------------------------------------------------------------------------------
class PSFExProcessor(object):

   """!
      Process a PSFEx catalog
   """
   ...
   def __init__(self, job_processor):
      ...
      self._job_processor = job_processor    # the underlying JobProcessor object
      self._helper = PpeHelper()             # ref. to utility class for convenience
   ...
   # -----------------------------------------------------------------------------------------------
   def process_catalog(self, pe_catalog_filepath, pe_output_check_filepath, file_type, job, worker):
      """!
         Apply various transformations to a PSFEx catalog
         @param pe_catalog_filepath full filepath of the catalog
         @param pe_output_check_filepath full path og check file to generate (if requested)
         @param file_type type of file to process
         @param job underlying Job object
         @param worker associated worker process
      """

      pe_xform_dico = {}   # result dictionary to populate
      ...
      try:
         ...
         initial_object_count = self.helper._get_catalog_nb_entries(pe_catalog_filepath,
                                                                    file_type, job, worker)

         if initial_object_count > 0:

            # --- Add extra columns to the catalog
            actual_object_count = self._add_extra_colunmns(pe_catalog_filepath, file_type,
                                     pe_output_check_filepath, job, worker)
      ...
      #         deleted_coords.extend(deleted_shifts)
               ...
               final_object_count = self._sort_by_coordinates(pe_catalog_filepath,
                                                              file_type, job, worker)

               if final_object_count > 0:
                  # --- If requested, create a mosaic containing the remaining (non deleted) objects
                  #     referenced in the final PSFEx catalog. The relative positions of the objects
                  #     are maintained.
                  if worker.config.get_as_boolean("CREATE_STAMP_MOSAIC", "DEBUGGING"):
                     self._create_stamp_mosaic(image_filepath,
                                               pe_catalog_filepath, deleted_coords,
                                             file_type, job, worker)

               #if worker.config.get_as_boolean("CREATE_STAMP_PLOT_MOSAIC", "DEBUGGING"):
               #   self._create_stamp_plot_mosaic(image_filepath, pe_catalog_filepath,
               #                                  file_type, job, worker)

               # --- Dictionary with the relevant information for later processing
               #pe_xform_dico["actual_object_count"]  = actual_object_count
               pe_xform_dico["initial_object_count"] = initial_object_count
               pe_xform_dico["duplicate_object_count"] = duplicate_object_count
               pe_xform_dico["flagged_object_count"] = flagged_object_count
               #pe_xform_dico["bad_centroids_object_count"] = bad_centroid_shift_count
               pe_xform_dico["final_object_count"] = final_object_count
               pe_xform_dico["elapsed_time"] = time.clock() - start_time

               if worker.logging_enabled():
                  if pe_xform_dico["final_object_count"] > 0:
                     worker.logger.log_info_p(
                         "{0} - /{1}/img-{2:03}-{3:1d} - {4} - "\
                         "Catalog {5} processed successfully - Final object count: {6}".format(
                         worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type,
                         pe_catalog_filepath, pe_xform_dico["final_object_count"]))
                  else:
                     worker.logger.log_warning_p(
                         "{0} - /{1}/img-{2:03}-{3:1d} - {4} - Catalog {5} is empty: "\
                         "all {6} entries have been deleted".format(
                         worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type,
                         pe_catalog_filepath, initial_object_count))

                  worker.logger.flush()
         else:
            # --- No objects were detecte by PSFEx
            if worker.logging_enabled():
               worker.logger.log_error_p(
                   "{0} - /{1}/img-{2:03}-{3:1d} - {4} - "\
                  "No object could be detected by PSFEx".format(
                   worker.name, job.get_branch_tree(), job.img_no, job.epoch, file_type) )
         ...
         if worker.logging_enabled():
            worker.logger.log_error_p(
                "{0} - An error occurred while processing PSFEx catalog: {1} ({2})".format(
                                                                                worker.name,
                                                                                job,
                                                                                detail))
            worker.logger.flush()

      return pe_xform_dico
   ...
   # -----------------------------------------------------------------------------------------------
   def _add_extra_colunmns(self, pe_catalog_filepath, file_type, pe_output_check_filepath,
                                 job, worker):
      ...
      try:
         ...
         # --- Open SE catalog
         catalog = self.helper._open_catalog(pe_catalog_filepath, job, worker)
   ...
   # -----------------------------------------------------------------------------------------------
   def _mark_centroids(self, pe_output_check_filepath, centroids, file_type, job, worker):

      if worker.config.get_as_string("PE_CHECKIMAGE_TYPE", "PSFEX") != "NONE" and \
         ...
         if mark_centroid_pix_value != -1:
            ...
            self.helper.mark_centroids(pe_output_check_filepath, centroids,
                                                                 mark_centroid_pix_value)

#   # -----------------------------------------------------------------------------------------------
#   def _mark_stamps(self, pe_output_check_filepath, coords, file_type, job, worker):
...
#            self.helper.mark_fits_stamps_file(pe_output_check_filepath, coords,
#                                              job.get_stamp_size(),
#                                              mark_stamp_pix_value, marking_shape="cross")


   # -----------------------------------------------------------------------------------------------
   def _handle_splitted_objects(self, pe_catalog_filepath, pe_output_check_filepath,
                                      file_type, job, worker):
      ...
      if splitted_obj_mgt == 'DELETE_ALL':
         ...
         plot_fragment_info = worker.config.get_as_boolean("CREATE_PE_PLOTS", "DEBUGGING")
      ...
      pe_output_cat_type = worker.config.get_as_string("PE_OUTPUT_CATALOG_TYPE", "PSFEX")
      if pe_output_cat_type in  ["FITS_1.0"]:
         # --- FITS format
         catalog = FITSCatalog(pe_catalog_filepath)
      ...
      if worker.config.get_as_string("PE_CHECKIMAGE_TYPE", "PSFEX") != "NONE" and \
         ...
         try:
            ...
            if mark_duplicated_objs_pix_value != -1:
               ...
               # Do the marking
               self.helper.mark_fits_stamps_file(pe_output_check_filepath, deleted_coords,
                                                 job.get_stamp_size(),
            ...
            deleted_centroid_coords = zip(yc_array[deleted_indice], xc_array[deleted_indice])
            remaining_centroid_coords = list(set(centroid_coords) - set(deleted_centroid_coords))
            self._mark_centroids(pe_output_check_filepath, remaining_centroid_coords,
                                 file_type, job, worker)
   ...
   # -----------------------------------------------------------------------------------------------
   def _filter_flagged_entries(self, pe_catalog_filepath, file_type, pe_output_check_filepath,
                                     job, worker):
      ...
      error_flags = [int(e) for e in worker.config.get_as_list("PE_ERROR_FLAGS", "CLEANING")]
      ...
      if len(to_delete) > 0:
         ...
         if worker.config.get_as_string("PE_CHECKIMAGE_TYPE", "PSFEX") != "NONE" and \
            ...
            if mark_flagged_objs_pix_value != -1:
               # Do the marking
               self.helper.mark_fits_stamps_file(pe_output_check_filepath, flagged_coords,
                                                 job.get_stamp_size(),
   ...
   # -----------------------------------------------------------------------------------------------
   def _sort_by_coordinates(self, pe_catalog_filepath, file_type, job, worker):
   ...
   def _create_stamp_mosaic(self, image_filepath,
                                  pe_catalog_filepath, deleted_coords, file_type, job, worker):
      ...
      mosaic_filename_pattern = worker.config.get_as_string("PE_OUPUT_FINAL_CHECK_FILENAME",
                                                            "PSFEX")
