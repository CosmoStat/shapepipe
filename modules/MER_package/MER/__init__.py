...
The Python program files are located in the ./MER/ directory and the
default configuration file
...
- Download and unpack the file MER-1.0.0.tar.gz in the directory of
  your choice
- MER is a pure Python module and can then be installed using the
  standard procedure:

  <ol>
   <li> create a console or terminal session </li>
   <li> cd to the directory where MER-1.0.0.tar.gz was unpacked </li>
...
The general syntax is:
  ...
  SMP version: MER_SMP.py [options]
  MPI version: MER_MPI.py [options]

by default, MER will look for a configuration file named config.cfg in
the ./MER/config directory. The location of this file can be changed
...
To run MER_MPI.py, one has to use the mpirun executable as:

   - <code>mpirun -np N MER_MPI.py [options]</code>
...
For example: 'mpirun -np 6 MER_MPI -d mydir -c myconfig.cfg' will run
...
Notes
-----
Before running MER, edit the configuration file to set the
