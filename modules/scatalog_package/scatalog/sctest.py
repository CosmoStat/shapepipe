...
def test():
   ...
   fits_cat = FITSCatalog("test/galaxy_catalog-001.fits", hdu_no=1)
   fits_cat.open()   
   print(fits_cat)
   ...
   print "Col values:", fits_cat.get_col_values()
   print "Col data('ID')", fits_cat.get_named_col_data("ID")
   ...
   fits_cat = FITSCatalog("test/galaxy_catalog-001.fits",
                           open_mode=FITSCatalog.OpenMode.ReadWrite, hdu_no=1)
   fits_cat.open()
# 
#    if fits_cat.col_exists("ID"):
#       fits_cat.remove_named_col("ID")
#    if fits_cat.col_exists("Col 2"):
#       fits_cat.remove_named_col("Col 2")

   col_1_data = np.random.rand(fits_cat.get_nb_rows())
   col_2_data = np.random.rand(fits_cat.get_nb_rows()) 
   fits_cat.add_col("Col 1", col_comment="New column 1", col_data=col_1_data)
   fits_cat.add_col("Col 2", col_comment="New column 2", col_data=col_2_data)
   ...
   np_fits_cat = FITSCatalog("test/empty_np_cat.fits", hdu_no=1)
   np_fits_cat.create()
   np_fits_cat.open()
   print np_fits_cat
   np_fits_cat.close()
   ...
   np_fits_cat = FITSCatalog("test/np_cat.fits", hdu_no=1)
   col_names = ["col 1", "col 2", "col 3", "col 4", "col 5"]
   np_fits_cat.create_from_numpy(matrix, col_names, ext_name="Objects", ext_ver=1)
