...
#      return (numpy.sum((stamp_border - numpy.median(stamp_border.flat))**2) / len(stamp_border.flat))**0.5
   ...
   def compute_stamp_wgauss_orthowave(self,stamp,medianFlag=True):
      ...
      PyMR= MRTrans('-t14 -n2 -L -T1') #(BI)ORTHOGONAL TRANSFORM
   ...
   # -----------------------------------------------------------------------------------------------
   #FCS FUNCTION ADDED
   def compute_alpha_coeffs_SNR(self, galaxy_stamp, sigma_noise_sq, 
                                   mrt_scale, blocksize,exclude_scalecoeff=False):
      prevbandsize =galaxy_stamp.shape
      for scale in range(mrt_scale):
         newbandsize= (prevbandsize / 2.0)
         diagdetails=galaxy_stamp[newbandsize[0]: prevbandsize[0], newbandsize[1]:prevbandsize[1]]
         hordetails=galaxy_stamp[newbandsize[0]: prevbandsize[0], 0:newbandsize[1]]
         vertdetails=galaxy_stamp[0: newbandsize[0], newbandsize[1]:prevbandsize[1]]
         write_as_fits(diagdetails, "./diagdet_"+str(scale)+".fits")
         write_as_fits(hordetails, "./hordet_"+str(scale)+".fits")
         write_as_fits(vertdetails, "./verdet_"+str(scale)+".fits")

   # -----------------------------------------------------------------------------------------------
   def _get_centroid_position(self,rel_centroid,abs_rel_centroid,stamp_size):
      rel_c=numpy.array(rel_centroid)
      start_index=rel_c-numpy.array([stamp_size,stamp_size])//2
      abs_rel_c=numpy.array(abs_rel_centroid) 
   ...
   def _get_psf_centroid_shift(self,rel_centroid,abs_rel_centroid,start_ind,stamp_size):
      ...
      psf_offset=psf_pos-numpy.array([psf_stamp_size,psf_stamp_size])//2 #center of stamp
      psf_shift = start_ind+ psf_offset
   ...
   def fftw_init(self,request,worker):
     ...
     """
      ...
      request._ifftw_plan = pyfftw.FFTW(request._fftwStamp2,request._ifftwStamp,
                              ...
                              'FFTW_DESTROY_INPUT'),direction='FFTW_BACKWARD')
	 ...
	 @param corr_pixwin correct for pixel window function in discrete convolution
	  ...
	  if galaxy_actual_stamp_size >0:
	     shift+=self._get_galaxy_centroid_shift(galaxy_rel_centroid,galaxy_abs_rel_centroid,
          ...
          fpsf=self.helper.perform_shift_in_frequency(fpsf,psf_stamp_ext.shape,
                                                  shift,request)
       elif (pixel_shift!=0):
          fpsf=self.helper.perform_shift_in_frequency(fpsf,psf_stamp_ext.shape,
                                                  pixel_shift,request)
       if(corr_pixwin):
	  fpsf=self.helper.correct_pixel_window_function(fpsf,psf_stamp_ext.shape,
                                                                      request)
	 ...
	 @param corr_pixwin correct for pixel window function in discrete convolution
 	 ...
 	 @param galaxy_abs_rel_centroid centroid position in galaxy stamp in fractions of pixel
       ...
       psf_stamp_ext[tuple(psf_slice)]=psf_stamp.astype(
                                                          request._prec_nump)
       fpsf =numpy.fft.rfftn(psf_stamp_ext)
       if (psf_shift):
	  ...
	  if galaxy_actual_stamp_size >0:
	     shift+=self._get_galaxy_centroid_shift(galaxy_rel_centroid,galaxy_abs_rel_centroid,
	                                            galaxy_actual_stamp_size)
 	  fpsf=self.helper.perform_shift_in_frequency(fpsf,fsize,shift,request)
       elif(any(pixel_shift != 0.)):
	  fpsf=self.helper.perform_shift_in_frequency(fpsf,fsize,
                                                  pixel_shift,request) 
       if(corr_pixwin):
          fpsf=self.helper.correct_pixel_window_function(fpsf,fsize,request)
   ...
   def scipy_preprocess_psf(self,psf_stamp,psf_shift,pixel_shift,corr_pixwin,psf_rel_centroid,
	 ...
	 @param corr_pixwin correct for pixel window function in discrete convolution
 	 ...
 	 @param galaxy_abs_rel_centroid centroid position in galaxy stamp in fractions of pixel
       ...
       if (psf_shift):
          fpsf =numpy.fft.rfftn(psf_stamp)
          shift = self._get_psf_centroid_shift(psf_rel_centroid,
		               psf_abs_rel_centroid_dico,0,psf_actual_stamp_size)
 	  if galaxy_actual_stamp_size >0:
	     shift+=self._get_galaxy_centroid_shift(galaxy_rel_centroid,galaxy_abs_rel_centroid,
	                                            galaxy_actual_stamp_size)
	  fpsf=self.helper.perform_shift_in_frequency(fpsf,fsize,
	     ...
	     print("CORRECTION PIXWIN")
          ifpsf =numpy.fft.irfftn(fpsf, fsize).copy()
   ...
   def perform_shift_in_frequency(self, fpsf, size_img,shift,request):
      ...
      if phase_factor[1] ==0.:
         ...
         kx_ft=numpy.exp(numpy.fft.fftfreq(size_img[0],d=-1./phase_factor[1])*1j)
      if phase_factor[0] ==0.:
         ky_ft=numpy.zeros(fpsf.shape[1],dtype=request._prec_nump)+1.
      else:
         if fpsf.shape[1] != size_img[1]:
            ky_ft=numpy.exp(numpy.fft.rfftfreq(size_img[1],d=-1./phase_factor[0])*1j)
         else:
            ky_ft=numpy.exp(numpy.fft.fftfreq(size_img[1],d=-1./phase_factor[0])*1j)
   ...
   def fftwconvol_real(self,fft_psf, Img,request):
      ...
      """
      #self.write_as_fits(Img,"output/model_galaxy_000-0.fits")
      Img_ext= numpy.zeros(request._inputStamp.shape, 
                                              dtype=request._prec_nump)
      Img_ext[request._gal_slice]=Img.astype(request._prec_nump)
      #request._inputStamp[:]= numpy.fft.ifftshift(Img_ext)
      request._inputStamp[:]= Img_ext
      #self.write_as_fits(numpy.fft.ifftshift(Img_ext),"output/bef_convolved_galaxy_000-0.fits")
      del Img_ext
      request._fftw_plan.execute()
      copyfftw=copy.deepcopy(request._fftwStamp)
      request._fftwStamp[:] = copyfftw * fft_psf
      request._ifftw_plan.execute()
      gal_conv =numpy.empty_like(request._ifftwStamp)
      gal_conv[:]= copy.deepcopy(request._ifftwStamp) 
      #self.write_as_fits(gal_conv/ request._fftw_norm, "output/full_convolved_galaxy_000-0.fits")
      #gal_conv =numpy.roll(gal_conv, request._gal_conv_center[0]-1,axis=0) #GREAT3 convention: center at 23 for stamp size 48, vs fftshift convention center at 24
      #gal_conv =numpy.roll(gal_conv, request._gal_conv_center[1]-1,axis=1)
      gal_conv= gal_conv[request._gal_conv_slice]
      tempConv=gal_conv/ request._fftw_norm
      return copy.deepcopy(tempConv)
   ...
   def fftmult_real_scipy(self,fPSF, img,request):
      ...
      shape=  numpy.asarray((shape[0],2*shape[1]-1))#real transform
      # Speed up FFT by padding to optimal size for FFTPACK
      fsize = shape.astype('int32')
      Img_ext=numpy.zeros(fsize,dtype=request._prec_nump)
      Img_ext[request._gal_slice]=img.astype(request._prec_nump)
      fConvol=numpy.fft.rfftn(numpy.fft.ifftshift(Img_ext))
      fTemp=copy.deepcopy(fConvol)
      del fConvol
      fTemp*=(fPSF)
      print("SHAPE FTEMP=",fTemp.shape,"GCCenter=",request._gal_conv_center,"GSlice=",request._gal_slice,"GCSlice=",request._gal_conv_slice)
      gal_conv= numpy.fft.irfftn(fTemp).copy()
      del fTemp
     # gal_conv =numpy.roll(gal_conv, request._gal_conv_center[0]-1,axis=0) #GREAT3 convention: center at 23 for stamp size 48, vs fftshift convention center at 24
     # gal_conv =numpy.roll(gal_conv, request._gal_conv_center[1]-1,axis=1)
      gal_conv= gal_conv[request._gal_conv_slice].real
      return gal_conv
