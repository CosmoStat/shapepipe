...
class FittingModel(BaseFittingModel):
   ...
   def __init__(self, config_dir='./config/models', config_filename="gcbdsersic.cfg"):
      """! 
         Construct a @c gcbdsersic FittingModel object 
         @param config_dir configuration directory of the method
         @param config_filename configuration directory of the method
      """  

      BaseFittingModel.__init__(self, name="gcbdsersic", 
   ...
   def out_of_bound_handler(self):
      ...
      """
      #print "***MFIT: gcbdsersic, getting  out_of_bound_handler..."
   ...
   def numerical_error_handler(self):
      ...
      """
      #print "***MFIT: gcbdsersic, getting  numerical_error_handler..."
...
#      print("Multifit: out-of-bound-handler invoked - "\
#            "Iter={0} Param name={1} Param value={2} Bounds={3}".format(
#                                                  iiter, param.name, param_value, bound_index))
      status = 0
   ...
   def numerical_error_handler(self, iiter, param, value, value_name):
      ...
      #print "***MFIT: in gcbdsersic numerical_error_handler", iiter, param, value, value_name
  ...
  # -----------------------------------------------------------------------------------------------
   def gcbdsersic(self, model, stamp_size, subscale, flux, dn, dre, bre, df, e1, e2, xc, yc):
      """!
      ...
      try:
         ...
         # --- Abort the fit
         #self.helper.print_error("gcbdsersic model: an error occurred: {0}".format(sys.exc_info()[1]))
         print("*** gcbdsersic model *** An error occurred: {0}".format(sys.exc_info()))
