...
class ModelParam(object):
   ...
   def __init__(self, name, guess, bounds=[None, None], delta_guess=None):
      """!
      ...
      self._delta_guess = delta_guess  # max allowed % variation of fitted value around the guess
   ...
   @property
   def delta_guess(self):
      """! @return the maximum allowed % variation of fitted value around the guess value"""   
      return self._delta_guess
   ...
   @delta_guess.setter 
   def delta_guess(self, delta_guess):
      """!     
         Set the maximum allowed % variation of fitted value around the guess value
         @param delta_guess  maximum allowed % variation of fitted value around the guess value
      """
...
#      self._model_options = diag_options     # diagnose directives (logging, stats, etc.)
      ...
      # --- Read catalog column mapping information if available
...
#      #print "BaseFittingModel:", [(p.name, p.bounds, p.guess_value) for p in model_params]
   ...
   def get_params_from_config(self):
      ...
      # --- Read default configuration to make sure all parameters get defined
         ...
         if self.default_config.has_section("MODEL.PARAMS"):
            ...
            for subsection in self.default_config.get_subsections("MODEL.PARAMS"): 
               ...
               if subsection in self.config.get_subsections("MODEL.PARAMS"): 
                  ...
                  # --- Maximum % variation around guess value
                  if self.config.has_key("max_%_guess", subsection):
                     delta_guess = self.config.get_as_float("max_%_guess", subsection)
                  else:
                     delta_guess = None

                  #print "*** MFIT: config, delta_guess=", name, delta_guess
                  ...
                  #print "*** MFIT: default config, delta_guess=", name, delta_guess

               params.append(ModelParam(name, guess, bounds, delta_guess)) 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   def check_fitted_params(self, (x, y), full_galaxy_image, actual_galaxy_image, 
      ...
      param_tuples = zip(self.params, param_values)
      
      # --- Check parameter ranges
      lower_valid = [ v >= p.bounds[0] for (p, v) in param_tuples if p.bounds[0] is not None ]
      upper_valid = [ v <= p.bounds[1] for (p, v) in param_tuples if p.bounds[1] is not None ]
      is_valid = numpy.logical_and(numpy.all(lower_valid), numpy.all(upper_valid))
      if is_valid:
         if self.check_centroids:
            # --- Check parameter centroid shifts
            x0 = (actual_galaxy_image.shape[1]-1.0)/2.0
            ...
            xc = param_values[self.get_param_names().index("xc")]
            yc = param_values[self.get_param_names().index("yc")]
            valid_centroids = (math.fabs(xc - x0) <= self.max_centroid_shift) and \
                              (math.fabs(yc - y0) <= self.max_centroid_shift)
            is_valid = numpy.all(numpy.asarray(valid_centroids))
            
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            #print "*** MFIT (x,y)", (x, y), "centroids:", (x0, y0), (xc, yc), "shift:", self.max_centroid_shift, valid_centroids
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

         #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
         #print "*** MFIT: (x,y)", (x, y), "full_galaxy_image.shape:", full_galaxy_image.shape, "actual_galaxy_image.shape:", actual_galaxy_image.shape, full_galaxy_image.shape == actual_galaxy_image.shape
         #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

         if is_valid and full_galaxy_image.shape == actual_galaxy_image.shape:      
            # --- Check % variations around parameter guess values
            lower_valid = [ v >= p.guess_value * (1.0 - p.delta_guess / 100.0) \
                            for (p, v) in param_tuples if p.guess_value is not None and p.delta_guess is not None ]
            upper_valid = [ v <= p.guess_value * (1.0 + p.delta_guess / 100.0) \
                            for (p, v) in param_tuples if p.guess_value is not None and p.delta_guess is not None ]
            is_valid = numpy.logical_and(numpy.all(lower_valid), numpy.all(upper_valid))
                 
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            #print "*** MFIT: (x,y)", (x, y), "check_fitted_params(), lower:", lower_valid, "upper:", upper_valid, "valid:", is_valid
            #print "    => fitted:", [ v for (p, v) in param_tuples], "guess:", [p.guess_value for (p, v) in param_tuples if p.guess_value is not None],\
            #            "min:", [ p.guess_value * (1.0 - p.delta_guess / 100.0) \
            #                 for (p, v) in param_tuples if p.guess_value is not None and p.delta_guess is not None ], \
            #            "max:", [p.guess_value * (1.0 + p.delta_guess / 100.0) \
            #                 for (p, v) in param_tuples if p.guess_value is not None and p.delta_guess is not None ]
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
class BaseFittingMethod(FittingElement):
   ...
   def __init__(self, name, 
      ...
      self._fitting_config = None      # fitting configuration object if any
...
#      self._model_fitting_func = model_fitting_func
   ...
   class FittingStatus(object):
      ...
      def __init__(self, method, residuals=None, errno=0, 
                   msg="", warn_msgs=[], custom_data=None, 
                   chi2=0, reduced_chi2=-1, stderr=None, xerror=None, nfev=0):
         """!
            ...
            @param chi2 reduced chi square (chi square over number of degrees of freedom)
            @param reduced_chi2 reduced chi square (chi square over number of degrees of freedom)
            @param standard error on each param (diag. terms of cov. matrix * sqrt(reduced chi^2))
            @param xerror covariance matrix diagonal sigmas
         ...
         self._residuals = residuals         # flattened residuals
         self._chi2 = chi2                   # chi^2
         self._reduced_chi2 = reduced_chi2   # reduced chi^2 (chi^2 / nb_dof)
         self._stderr = stderr               # array of standard errors for each parameter
         self._xerror = xerror               # array of covariance matrix diagonal sigmas
         self._nfev = nfev                   # number of function evaluations
      ...
      def __str__(self):
         ...
         descr += "- nb. func. evaluations: {0}\n".format(self.nfev)
         descr += "- chi^2: {0}\n".format(self._chi2)
         descr += "- reduced chi^2: {0}\n".format(self._reduced_chi2)
         if self.stderr is not None:
            descr += "- param. std. errors: {0}\n".format(self._stderr)
         if self.xerror is not None:
            descr += "- param. cov. errors: {0}\n".format(self._xerror)
         descr += "- residuals: {0}\n".format(self._residuals)
      ...
      @property
      def residuals(self):
         """! @return residuals of the fit (observed data - model data) / 1 sigma std error """
      ...
      @property
      def stderr(self):
         """! @return the standard errors on parameters """
         return self._stderr

      @property
      def xerror(self):
         """! @return the errors from the covariance matrix """
         return self._xerror
      ...
      def warn_msgs(self, warn_msgs):
         """!
            ...
            @param list of warning message texts 
         """
      ...
      def residuals(self, residuals):
         """!
            Set residuals of the fit 
            @param residuals residuals of the fit (observed data - model data) / 1 sigma std error 
         """
      ...
      def chi2(self, chi2):
         """! 
            Set the chi squared
            @param chi2 reduced chi squared  
         """
         self._chi2 = chi2

      @reduced_chi2.setter
      def reduced_chi2(self, reduced_chi2):
         """! 
            Set the reduced chi squared
            @param reduced_chi2 reduced chi squared  
         """
         self._reduced_chi2 = reduced_chi2

      @stderr.setter
      def stderr(self, stderr):
         """! 
            Set the standard errors on parameters
            @param stderr standard errors  
         """
         self._stderr = stderr

      @xerror.setter
      def xerror(self, xerror):
         """! 
            Set errors from the covariance matrix
            @param xerror errors from covariance matrix
         """
         self._xerror = xerror
      ...
      def custom_data(self, custom_data):
         """! 
            Set custom data collected by model fitting if any
            @param custom data data collected by model fitting
         """
