...
from isap_great3_sparse2d import details
...
class FittingMethod(BaseFittingMethod):
   ...
   def __init__(self, method_config_dir='./config/methods', 
      ...
      #print "*** self.__model_params_dico:",self.__model_params_dico["scsersic"]
      ...
      # --- Last created model data in the objective function (i.e. # best-fitted model data)
      self.__last_model_data = None
   ...
   def diag_options(self, diag_options):
      ...
      self._fitting_options = diag_options
   ...
   def fit(self, model, args):
      """! 
         ...
         @note in case of success the @c errno code in 
      ...
      if model.name in self.__model_params_dico:
         ...
         for param in model.params:
            ...
            if not self.__model_params_dico[model.name]["parinfo"][param.name]["fixed"]:  
...
#               param.bounds = None
      ...
      try:      
         ...
         print("*** kmpfit fitting error: {0} ({1})".format(sys.exc_info(), detail))
      ...
      self.fitting_status.custom_data = []
      
      if type(self.__last_model_data) == numpy.ndarray: 
         self.fitting_status.residuals = numpy.reshape(self.__minimizer.resid, 
                                                      self.__last_model_data.shape)   # residuals
      else:
         self.fitting_status.residuals = numpy.nan   # error: fitting failed

      self.fitting_status.custom_data.append([self.__last_model_data, 
                                             self.fitting_status.residuals])  
      self.fitting_status.chi2 = self.__minimizer.chi2_min           # chi^2
      self.fitting_status.reduced_chi2 = self.__minimizer.rchi2_min  # reduced chi^2
      self.fitting_status.stderr = self.__minimizer.stderr           # std errors on each parameter
      self.fitting_status.xerror = self.__minimizer.xerror           # param errors from cov matrix
      self.fitting_status.nfev = self.__minimizer.nfev               # nb. of function evaluations
   ...
   def chi2_objective_func(self, params, data):
      ...
      """
      fitting_func, model, obs_data, sigma_noise, args = data

#      if math.hypot(e1, e2) >= 1.0:
#         e1 = e2 = 0.0
      
      self.__last_model_data = fitting_func(params, model, *args)
      residuals = (obs_data - self.__last_model_data) / sigma_noise

      #print "chi2:",  numpy.sum((obs_data - self.__last_model_data)**2), "rchi:", numpy.sum(residuals**2)/1017.0 
      #print "*** params:", params, numpy.sum(model_data), numpy.sum(obs_data), numpy.sum(residuals), residuals
