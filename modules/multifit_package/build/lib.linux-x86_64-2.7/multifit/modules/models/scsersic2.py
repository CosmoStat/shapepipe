...
class FittingModel(BaseFittingModel):
   ...
   def __init__(self, config_dir='./config/models', config_filename="scsersic2.cfg"):
      """! 
         Construct a @c scsersic2 FittingModel object 
         @param config_dir configuration directory of the method
         @param config_filename name of the configuration file
      """  

      BaseFittingModel.__init__(self, name="scsersic2", 
   ...
   def out_of_bound_handler(self):
      ...
      """
      #print "***MFIT: in scsersic2, getting out_of_bound_handler()"
   ...
   def check_fitted_params(self, (x, y), full_galaxy_image, actual_galaxy_image, 
      ...
      if is_valid:
         ...
         q = param_values[self.get_param_names().index("q")]
         e = (1.0-q)/(1.0+q)
         is_valid = e <= 1.0
   ...
   #      status = 0  # continue fitting
      ...
      #print "***MFIT: in scsersic2 out_of_bound handler", iiter, param, param_value, bound_index
   ...
   # -----------------------------------------------------------------------------------------------
   def scsersic2(self, model, stamp_size, subscale, flux, n, re, q, pa, xc, yc):
      """!
      ...
      radii  = numpy.hypot(X, Y) * numpy.sqrt(1.0 + (1.0/q**2 - 1.0) * numpy.sin(thetas-pa)**2) 
      ...
      try:
         ...
         print("*** scsersic2 model *** An error occurred: {0}".format(sys.exc_info()))
...
#   # -----------------------------------------------------------------------------------------------
#   def scsersic2(self, model, stamp_size, subscale, flux, n, re, e1, e2, xc, yc):
#      """!
#         Compute the model using specified parameters and arguments.
#         In this case, compute the profile @c profile_func on a square grid of size @c grid_size
#      """
