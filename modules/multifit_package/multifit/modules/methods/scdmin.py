...
#         self.objective_func = self.chi2_objective_func  # template chi^2 function by default
   ...
   def fitting_options(self, fitting_options):
      ...
      scdm.Minimizer.fitting_options = scdm_fitting_options
   ...
   def fit(self, model, args):
      ...
      if model.name in self.__model_params_dico:
         ...
         for param in model.params:
            ...
            if not self.__model_params_dico[model.name]["params"][param.name].is_constant:
               ...
               # --- Tell the minimizer if the parameter is constant or varying
      ...
      self._fitting_info = FittingMethod.FittingInfo(self, 
                                                     ...
                                                     self.__minimizer.nb_free_params, 
      ...
      self.fitting_status.nfev = self._fitting_info.nb_fev
      ...
      self.fitting_status.chi2 = self._fitting_info.chi2
      self.fitting_status.reduced_chi2 = self._fitting_info.reduced_chi2
      self.fitting_status.stderr = None   # no available standard errors on parameters
      ...
      if errno >= 0:
         ...
         # --- Maximum of iterations or function evaluations reached => still considered as success
   ...
   # -----------------------------------------------------------------------------------------------
   def chi2_objective_func(self, params, fitting_func, model, obs_data, sigma_noise, args):
      """!
         Chi2 template objective function 
      """

      model_data = fitting_func(params, model, *args)
      residuals = (obs_data - model_data) / sigma_noise
...
#            "Sum Data:", numpy.sum(model_data)
      ...
      return [numpy.sum(residuals**2), [model_data, residuals]] 
   ...
   def chi2_r_objective_func(self, params, 
      ...
      if rlambda > 0:
         ...
         residuals = (obs_data - model_data) / sigma_noise_sq + \
...
#            "Sum Data:", numpy.sum(model_data)
   ...
   class FittingInfo(BaseFittingInfo):
      ...
      def __init__(self, method, 
                         errno, nb_iter, nb_fev, nb_free, msg, warn_msgs, 
         ...
         """!
            ...
            @param nb_free number of free (non fixed) parameters
         ...
         self._nb_free = nb_free                # number of free (non fixed) parameters
         ...
         if not self._custom_data is None and len(self._custom_data) > 0:
            self._model_data = numpy.ravel(custom_data[0][0])     # last model data
            self._residuals = numpy.ravel(custom_data[0][1])      # residuals
            self._nb_dof = len(self._model_data) - self._nb_free # TODO deal with fixed params
            self._chi2 = numpy.sum(self._residuals**2) 
            self._reduced_chi2 = numpy.sum(self._residuals**2) / self._nb_dof 
         else:
            self._model_data = None
            self._residuals = None
            self._nb_dof = -1
            self._chi2 = -1   
            self._reduced_chi2 = -1   

         #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
         # print "SCDMIN NB_DOF:", self._nb_dof   
         # print "SCDMIN NB_FREE:", self._nb_free
         # print "SCDMIN CHI2:", self._chi2
         # print "SCDMIN RED_CHI2:", self._reduced_chi2
         #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      def nb_free(self):
         """! 
            @return the number of free (non fixed) parameters
            @note refer to the corresponding @c SCDM documentation
         """
      ...
      def reduced_chi2(self):
         """! 
            @return the reduced_chi squared
            @note refer to the corresponding @c SCDM documentation
         """
